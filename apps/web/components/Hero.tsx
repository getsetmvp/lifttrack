        ...
        <div className="grid items-center gap-12 lg:grid-cols-[1.05fr_0.95fr] lg:gap-16">
          <div className="animate-fade-up">
            ...
            <h1 className="mt-6 text-5xl font-bold leading-[1.05] tracking-tight md:text-6xl lg:text-[68px]">
              <span className="block">Train smart.</span>
              <span className="block">
                Eat <span className="gradient-text">smarter.</span>
              </span>
            </h1>

            <p className="mt-6 max-w-xl text-lg leading-relaxed text-muted dark:text-muted-dark">
              LiftTrack is the lifting log + nutrition tracker built for serious progress. Log every rep —
              sets, decimal weights, drop sets — and snap any meal to get macros from AI in seconds. See
              why your progress moves, not just that it did.
            </p>
            ...
            <dl className="mt-12 grid max-w-md grid-cols-3 gap-4 text-sm">
              <Stat label="Meal slots / day" value="7" />
              <Stat label="Tap to log set" value="<3s" />
              <Stat label="Ads ever" value="0" />
            </dl>
          </div>

          <div className="relative flex justify-center lg:justify-end">
            <div className="absolute -inset-8 -z-10 rounded-[60px] bg-gradient-to-br from-brand/30 via-accent/20 to-transparent blur-3xl" />
            <div className="flex items-end gap-4 md:gap-6">
              <PhoneFrame
                src="/screenshots/today.png"
                alt="Today tab — routine + macros + streak"
                width={220}
                className="translate-y-6 md:translate-y-10"
              />
              <PhoneFrame
                src="/screenshots/workout.png"
                alt="Active workout — set logger"
                width={260}
                priority
              />
            </div>
